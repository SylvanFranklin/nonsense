...
#let buzzword(i) = {
    return get((
        ...
        "connected", "hyprebolic", "anti-standard", "maximal", "congruential"
    ), i)
}
...
#let field = (i, capital: false, title-case: false) => {
    let bases = (
        ...
        "graph theory", "game theory", "geometry", "zoncology" 
    )
    ...
    generation-symbol([fld], color: orange)[#{
        if capital [#bgs(cap(b1)) #bgs(b2) #base]
        else if title-case [#bgs(cap(b1)) #bgs(cap(b2)) #cap(base)]
        else [#bgs(b1) #bgs(b2) #base]
    }]
}
...
#let authors = (i) => {
    range(0, calc.rem(i, 4) + 1).map(n => 
        [#generation-symbol(n, color: gray)[#upper(get(alphabet, i + n))]. #generation-symbol(i+n, color: orange)[#get(last-names, i + n)]]
    ).join(", ")
}

#let theorem = (i, capital: false, title-case: false) => {
    let b = buzzword(i)
    let a = if calc.rem(i, 2) == 0 { get(last-names, i) }
    else { buzzword(i - 2) }
    let kind = get(("lemma", "theorem", "axiom", "conjecture", "principle", "extension", "theory"), i)

    generation-symbol([thm], color: orange)[#{
        if capital [The #b #a #ogs(object(i)) #kind]
        else if title-case [The #bgs(cap(b)) #cap(a) #ogs(object(i)) #cap(kind)]
        else [the #bgs(b) #a #ogs(object(i)) #kind]
    }]
}
...
#let property(i, case) = {
    ...
    generation-symbol(case, color: olive)[#{
        ...
        else if case == 1 [is #ogs(sing(object(i)))]
...
#let non-setup(i, case) = {
    ...
    generation-symbol(case, color: navy)[#{
        if case == 0 [Fix #sing(buzz1) #buzz2 #object(i), #eq1 in #sing(buzz3) #object(i+1), #eq2.]
        else if case == 1 [Let #sing(buzz2) #object(i) that #prop, be defined #eq2]
        else if case == 2 [Fix #sing(buzz1) #object(i+1) in #sing(buzz2) #buzz3 #object(i+1).]
        else if case == 3 [Assume #sing(buzz1) #sing(buzz2) #object(i) #prop.]
...
#let non-statement = (i, case) => {
        ...
        let person = generation-symbol(calc.rem(i*3, 6))[#get(("self respecting", "well educated", "logical", "sane", "competent"), i*3)]
        ...
        let last-name = generation-symbol(calc.rem(i, last-names.len()))[#get(last-names, i)]
        let res = theorem(i*3)

        context {
            let reference = generation-symbol(-1, color: orange)[[#{1 + calc.rem(i * case * 979, references.get())}]] 
        ...
        #{
            ...
            else if case == 16 [The work of #last-name on #res #reference is #glaze for #adverb2 #stem\ing #bgs(sing(buzz1)) #ogs(obj1), which #adverb1 #property.]
            // Inline text
            else if case == 7 [By #stem\ing #bgs(sing(buzz1)) #ogs(obj1) on a #ogs(obj2), that is #eq-small(case+i, i+case*3) We reach #bgs(sing(buzz3)) #bgs(buzz2) #ogs(obj3)).]
            else if case == 8 [#bgs(cap(sing(buzz2))) #adverb1 #property, provided #eq-small(i, i*case+3)#eq-small(i + 1, case*i*83)]
            else if case == 9 [However, #eq-small(i, case*i*3), as shown #adverb2 in #reference is #bgs(sing(buzz2)) #ogs(obj3) and #property.]
            ...
            // medium
            else if case == 15 [Provided #eq-small(i, i*case*3), as shown in #reference any #person would agree that every #ogs(obj1) #property]
...
#let proof-theorem-lemma(i, heading) = {
    ...
    #let reference(i, case) = {
        ...
        let title = generation-symbol(case, color: eastern)[#{
            ...
            else if case == 1 [On the #bgs(cap(buzzword(i))) #ogs(cap(object(i) + "s"))]
            else if case == 2 [#theorem(i, title-case: true) and Applications]
            else if case == 3 [On #field(i, title-case: true) and #ogs(object(i))]
            else if case == 4 [#bgs(cap(buzzword(i))) #field(i, title-case: true)]
            else if case == 5 [#bgs(cap(buzzword(i))) Methods in #field(i, title-case: true)]
        }]
        ...
        let publisher(i, case) = {
            ...
            generation-symbol(case, color: olive)[#{
                ...
                else if case < 12 [Journal of #bgs(buzzword(i)) #ogs(object(i))]
...
#let nonsense(body) = {
    ...
    par(hanging-indent: -2em, justify: true)[
        #{for (i, c) in chars.enumerate().slice(0, calc.min(chars.len(), 65)) {
            ...
            if i == 0 { 
                count.step()
                context generation-symbol(count.get().first(), color:
